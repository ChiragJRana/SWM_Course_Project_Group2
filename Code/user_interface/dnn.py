...
def get_movie_recommendations(user_id):
    ...
    user_rated_movies = movie_data[movie_data['user_id']
                                   == user_id]['movie_id']
    ...
    recommendation_data = pd.DataFrame(
        {'user_id': user_ids, 'movie_id': movies_to_recommend})
    ...
    for index, row in top_recommendations.iterrows():
        ...
        movie_title = movie_data[movie_data['movie_id']
                                 == movie_id]['movie_title'].iloc[0]
...
model = load_model()
# Load the movie data
movie_data = pd.read_csv('dnn_movie_list.csv', usecols=['user_id', 'movie_id', 'movie_title'], dtype={
                         'user_id': 'int32', 'movie_id': 'int16', 'movie_title': 'object'})
movie_data['user_id'] = movie_data['user_id'].astype(
    'category').cat.codes.values
movie_data['movie_id'] = movie_data['movie_id'].astype(
    'category').cat.codes.values
