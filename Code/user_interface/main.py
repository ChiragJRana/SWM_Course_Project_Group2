...
def index():
    if request.method == 'POST':
        ...
        table_data = [(row['movie_id'], row['movie_title'],
                       row['movie_rating']) for index, row in tdf.iterrows()]
        option_text = {'A': 'K-Means',
                       'B': 'Matrix Factorization', 'C': 'Deep Neural Network'}
        selected_option_text = option_text.get(
...
def generate_table_data(selected_option, entered_number):
    ...
    if selected_option == 'A':
        ...
        # data = [(f'Option {selected_option} - Row {i}',
        #  entered_number+i) for i in range(1, 6)]
        # data = get_dnn_prediction(entered_number)
        data = get_dnn_prediction(entered_number)
    return data
...
def get_dnn_prediction(pid):
    data_k = kmeans_model.get_temp_prediction(int(pid))
